...
mod tests {
    ...
    pub fn blake3_to_cid(hash: iroh_blobs::Hash) -> Cid {
        ...
        let mh = Multihash::wrap(0x1e, hash.as_bytes()).expect("invalid BLAKE3 hash");
    ...
    async fn test_resolve() {
        ...
        let crp = IrohCrp::new_from_config(
            IrohCrpConfig {
                node_addr_ref: IrohNodeAddrRef::NodeTicket(ticket.to_string()),
            },
            ProviderConfig::Iroh(IrohCrpConfig {
                node_addr_ref: IrohNodeAddrRef::NodeTicket(ticket.to_string()),
            }),
        )
        .await
