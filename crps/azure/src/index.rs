...
use iroh_blobs::{Hash, format::collection::Collection};

use cid_router_core::{Context, crp::ProviderType, routes::Route};
...
impl Indexer {
    ...
    pub async fn update_blob_index(&self, blob_storage_config: &BlobStorageConfig) -> Result<()> {
        ...
        for container_cfg in &blob_storage_config.containers {
            self.add_index_entries_for_missing_blobs(cx, container_cfg)
                .await?;
    ...
    async fn add_index_entries_for_missing_blobs(&self, cfg: ContainerConfig) -> Result<()> {
        ...
        for blob in response.blobs.blobs() {
            ...
            if self
                ...
                .is_none()
            {
                let route = Route::builder(self.cx.secret_key(), ProviderType::Azure)
                    .size(size)
                    .route(url)
                    .cid(todo!())
